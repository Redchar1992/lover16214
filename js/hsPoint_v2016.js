$(function(){
	...
	//画布控制
	var topHeight = screenWidth*0.35;//上部高度
	...
	$(".white,.border").css({"height":screenHeight-topHeight});
	//判断画左边还是画右边
	function areaJudge(statu){
		if(statu==0){
			$(".white").css("left",screenWidth/2);
			brd.css("left",(parseInt(screenWidth)-320)/2-2+"px");
			$(".border").css("left",(parseInt(screenWidth)-320)/2-2+"px");
			$(".mask").css("left",(parseInt(screenWidth)-320)/2+"px");
			//上方提示语
			$(".ab_left,.ab_right").show();
			$(".cd_left,.cd_right").hide();
		}
		else if(statu==1){
			$(".white").css("left",0);
			brd.css({"left":parseInt(screenWidth)/2-2+"px"});
			$(".border").css({"left":parseInt(screenWidth)/2-2+"px","border-right":"none","border-left":"2px dashed #a2a1a1"});
			...
			//上方提示语变更
			$(".ab_left,.ab_right").hide();
			$(".cd_left,.cd_right").show();
		}
	}
	areaJudge(stt);
	...
	var move = function(e){
		brd.on('touchstart' , function (e) {
			...
			disY = e.targetTouches[0].pageY - this.offsetTop-topHeight;
		...
		brd.on('touchmove' , function (e) {
			...
			mouseY = e.targetTouches[0].pageY - this.offsetTop-topHeight;
...
//---------------------------已结束-----------------------------------------
	...
	//下载慧车宝
	$(".downHcb").tap(function(){
		window.location.href = 'http://m.51hcb.com/';
	})
})